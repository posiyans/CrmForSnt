...
class PrimaryReceiptPdfClass extends PrimaryPdfClass
{

    protected $stead_number;
    protected $pages;
    protected $name;
    protected $inn;
    ...
    public function __construct()
    {
        ...
        $this->setGlobalVar($options);
        $this->pages = [];
    }


    public static function forInvoices(BillingInvoiceModel|Collection $invoices, $addPayer = false)
    {
        $pdf = new self();
        if ($invoices instanceof Collection) {
            foreach ($invoices as $invoice) {
                $pdf->forInvoice($invoice, $addPayer);
            }
            $pdf->setSteadNumber('');
        } else {
            $pdf->forInvoice($invoices, $addPayer);
        }
        return $pdf;
    }

    public function setSteadNumber($stead_number)
    {
        $this->stead_number = $stead_number;
        return $this;
    }

    public function forInvoice(BillingInvoiceModel $invoice, $addPayer = false)
    {
        $options = [
            'stead_num' => $invoice->stead->number ?? '',
        ...
        if ($addPayer) {
            $stead = $invoice->stead;
            $options['last_name'] = $stead->owners[0]->getValue('surname', '') ?? '';
            $options['first_name'] = $stead->owners[0]->getValue('name', '') ?? '';
            $options['middle_name'] = $stead->owners[0]->getValue('middle_name', '') ?? '';
        }
        $this->stead_number = $invoice->stead->number ?? '';
        $this->addPage($options);
        return $this;
    }


    public function response($fileName = false)
    {
        $tmpfname = tempnam(sys_get_temp_dir(), 'ticket_stead');
        $this->Output($tmpfname);
        if (!$fileName) {
            $fileName = count($this->pages) > 1 ? 'Квитанции' : 'Квитанция';
            if ($this->stead_number) {
                $fileName .= '_участок_' . $this->stead_number;
            }
            $fileName .= '.pdf';
        }
        ...
        $fileName = str_replace('"', '', $fileName);
        return response()->download($tmpfname, $fileName);
    }
    ...
    public function render()
    {
        foreach ($this->pages as $page) {
            ...
            $this->fillUserData($page);
            if ($this->qr_code) {
                $this->insertQRcode($page);
            }
        }
