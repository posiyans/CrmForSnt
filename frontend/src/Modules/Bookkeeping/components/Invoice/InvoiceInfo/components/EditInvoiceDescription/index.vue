...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const func = (data) => {
      ...
      props.invoice.options[props.fieldName] = data.value
      return updateInvoiceField(props.invoice.id, data)
    }
    onMounted(() => {
      data.value = props.invoice.options[props.fieldName]?.replaceAll('@', "\n") || ''
    })
    const setValue = (data) => {
      props.invoice.options[props.fieldName] = data.replaceAll("\n", '@')
      emit('reload')
    }
    return {
      data,
