<template>
  <div class="row items-center" style="max-width: 45vw;">
    <ComponentField v-if="edit" v-model="newValue" :item="field" dense outlined class="text-grey-7" />
    <div v-if="!edit" class="">
      <div v-if="field.type === 'date'">
        <ShowTime :time="modelValue" format="DD-MM-YYYY" />
      </div>
      <ShowBooleanType v-else-if="field.type === 'boolean'" :model-value="modelValue" />
      <div v-else>
        {{ modelValue }}
      </div>
    </div>
...
import ShowBooleanType from './ShowBooleanType.vue'
